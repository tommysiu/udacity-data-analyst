...
// disable geomap built-in clicked function
...
// function to highlight the bar chart item and corresponding state
var highlight_item = function(rect) {
  var bar = d3.select('.bars').select('#' + rect.id);
  var active = !bar.classed('active');

  // update the bar items
  d3.select('.bars').selectAll('rect')
    .classed('active', false);
  bar.classed('active', active);

  // update the map state color
  if(active) {
...
d3.jsonAsync('data/states_hash.json').then(function(dict){
  ...
  svg.select('#xaxis').call(x_axis)
    ...
    .attr('transform', 'translate(' + (bar_width/2 - 50) + ',-10)') 
    .text('Amount');
